(function ($) {
    ...
    function sectionsScripts() {
        ...
        /** Newsletter Form **/
        (function () {
            // Set up listener for newsletter form.
            $('#newsletter').submit(function (e) {
                // Stop the browser from submitting the form.
                e.preventDefault();

                var form = $(this);
                var formMessages = form.find('.form-errors');
                ...
                $.ajax({
                    ...
                    .done(function (response) {
                        ...
                        if (response.Error) {
                            ...
                            // Set the message text.
                            formMessages.text("Unable to sign up for newsletter. A detailed error has been logged and a solution is underway.");
                        }
                    })
                    .fail(function (data) {
                        // Make sure that the formMessages div has the 'error' class.
        ...
        (function () {
                    ...
                    .done(function (response) {
                        if (response.Success) {
                            ...
                            if (form.id == "newPost") {
                                ...
                                form.find('#categories').val('');
                                form.find('#imageUrl').val('');
            ...
            // Set up listener for new blog post form.
            $('#newBlog').submit(ajaxSubmit);

            // Set up listener for edit blog post form.
            $('#editBlog').submit(ajaxSubmit);
        })();
    }
    ...
    function scrollReveal(items) {
        ...
        $.each(items, function (itemKey, reveal) {
            $(reveal.selector).each(function (index, elem) {
                ...
                var revealData = {
                    duration: (typeof data.animationDuration != "undefined") ? parseInt(data.animationDuration) : (reveal.data.duration || 1000),
                    origin: (typeof data.animationOrigin != "undefined") ? data.animationOrigin : (reveal.data.origin || 'bottom'),
                    distance: (typeof data.animationDistance != "undefined") ? data.animationDistance : (reveal.data.distance || '0px'),
                    delay: (typeof data.animationDelay != "undefined") ? parseInt(data.animationDelay) : (reveal.data.delay || 0),
                    scale: (typeof data.animationScale != "undefined") ? parseFloat(data.animationScale) : (reveal.data.scale || 1),
                    rotate: (typeof data.animationRotate != "undefined") ? data.animationRotate : (reveal.data.rotate || {
                        x: 0,
                        y: 0,
                        z: 0
                    }),
                    easing: (typeof data.animationEasing != "undefined") ? data.animationEasing : (reveal.data.easing || 'cubic-bezier(1.000, 1.000, 1.000, 1.000)'),
                    mobile: false,
    ...
    function progressbarConfig(data) {
        return {
            ...
            from: (typeof data.progressbarFrom != "undefined") ? data.progressbarFrom : {
                color: '#aaa',
                width: 1
            },
            to: (typeof data.progressbarTo != "undefined") ? data.progressbarTo : {
                color: '#333',
                width: 4
            },
